...
OpenSource Globalizer AI 通过自动化以下工作，帮助**开源维护者构建真正全球化的社区**：

- 📖 **README / 文档翻译** — 通过 goldmark AST 在 10+ 种语言中保留 Markdown 结构
- 🔄 **GitHub Action 集成** — 推送时自动翻译，并自动发起 PR
...
开源本质上就是全球化的。你的用户使用中文、日本語、한국어、Español、Français、Deutsch…
但大多数维护者无法手动翻译每份 README、每个 Issue、每篇发布说明。
...
| 功能 | 版本 | 状态 | 描述 |
|---------|-----|--------|-------------|
| 📖 **README 翻译器** | v0.1 | ✅ 已发布 | 将 README.md 翻译为多种语言，goldmark AST 保留全部格式 |
| 🌐 **HTTP API** | v0.1 | ✅ 已发布 | 基于 Gin 的 REST API，POST /api/v1/translate |
| 🔄 **GitHub Action** | v0.2 | ✅ 已发布 | 推送时自动翻译，自动创建 PR |
...
> 📖 完整安装指南：**[INSTALL.md](INSTALL.md)** — 5 分钟入门指南。
...
> **前提条件**：`-v $(pwd):/workspace` 将当前目录挂载到容器中，因此**请从包含 `README.md` 的目录运行**。
> macOS Docker Desktop 默认仅共享 `/Users` 等目录 — 如果你从 `/tmp` 或其他未共享路径运行，容器将看不到你的文件（`no such file or directory`）。
...
> 通过 `base-url` 和 `model` 输入支持 OpenAI 兼容端点（例如 DeepSeek）。
> 测试时没有 API 密钥？将 `api-key` 留空并添加 `mock: true` 即可端到端验证流程。
